...
def obtener_clasificacion(liga_code):
    try:
        res = requests.get(f"{BASE_URL}competitions/{liga_code}/standings", headers=HEADERS, timeout=10)
        if res.status_code == 200:
            ...
            if standings:
                ...
                for item in table:
                    rows.append({
                        ...
                        "PJ": item.get("playedGames"),
                        "PG": item.get("won"),
                        "PE": item.get("draw"),
                        "PP": item.get("lost"),
                        "GF": item.get("goalsFor"),
                        "GC": item.get("goalsAgainst"),
                        "DG": item.get("goalDifference"),
                        "Pts": item.get("points")
                    })
...
# 6. PESTAÑAS
# -----------------------------------------------------------------------------
tab_marcad, tab_corners_tiros, tab_combinada, tab_btts_ou, tab_clasif, tab_value, tab_form = st.tabs([
    "🎯 Marcador & 1X2", 
    "🚩 Córneres y Tiros",
...
with tab_clasif:
    st.subheader(f"📊 Clasificación General — {LIGAS.get(liga_sel, liga_sel)}")
    df_clasificacion = obtener_clasificacion(liga_sel)
    
    if not df_clasificacion.empty:
        def resaltar_equipos(row):
            if row["Equipo"] == local_nom:
                return ['background-color: #1a3a4b; color: #00e5ff; font-weight: bold;'] * len(row)
            elif row["Equipo"] == visitante_nom:
                return ['background-color: #3b2a1a; color: #f59e0b; font-weight: bold;'] * len(row)
            return [''] * len(row)

        df_styled = df_clasificacion.style.apply(resaltar_equipos, axis=1)
        st.dataframe(df_styled, use_container_width=True, hide_index=True, height=600)
    else:
        st.warning("No hay datos de clasificación disponibles para esta liga en este momento.")
